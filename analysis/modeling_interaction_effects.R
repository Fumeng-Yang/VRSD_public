...
if(REBUILD){ 
  ...
  factor_abserror_fit <- brm(bf(AbsError ~ Stereo:Motion + Stereo:Perspective + Stereo:Motion + Stereo:Shading + Stereo:Dimen + Stereo:Dataset +
                                  Motion:Perspective + Motion:Shading +  Motion:Dataset + 
                                  Perspective:Shading + Perspective:Dataset + 
                                  Shading:Dataset+
                                  Dimen:Dataset+
                                  (1|ParticipantID), 
                                sigma ~ Stereo:Motion + Stereo:Perspective + Stereo:Motion + Stereo:Shading + Stereo:Dimen + Stereo:Dataset +
                                  Motion:Perspective + Motion:Shading + Motion:Dataset + 
                                  Perspective:Shading + Perspective:Dataset + 
                                  Shading:Dataset+
                                  Dimen:Dataset), 
  ...
  factor_reactime_fit <- brm(bf(ReactTime ~ Stereo:Motion + Stereo:Perspective + Stereo:Motion + Stereo:Shading + Stereo:Dimen + Stereo:Dataset +
                                  Motion:Perspective + Motion:Shading +  Motion:Dataset + 
                                  Perspective:Shading + Perspective:Dataset + 
                                  Shading:Dataset+
                                  Dimen:Dataset+
...
get_coef_plot <- function(factor_fit, ytitle, mu_scale, sd_scale, fcolor){
  ...
  g_var_mu <- g %>%
    ...
    + geom_point(data = coefs_ind, aes(x = Q50, y = b_ind), size = LINE_SIZE_WIDE + LINE_SIZE_NARROW, color = "black")%>% # , position = position_nudge(y = 0.35)
    ...
    + scale_x_continuous(limits=mu_scale, breaks = seq(-0.5, 0.5, by = 0.05), expand = c(0.01,0.01))%>%
  ...
  g_var_sd <- g %>%
    ...
    + geom_point(data = coefs_sigma, aes(x = Q50, y = b_ind), size = LINE_SIZE_WIDE + LINE_SIZE_NARROW, color = "black")%>% # , position = position_nudge(y = 0.35)
    ...
    + geom_point(data = coefs_sigma, aes(x = Q50, y = b_sigma), size = LINE_SIZE_WIDE, color = "black" )%>% 
    + geom_text(data = coefs_sigma, size = 2, aes(x = max(`Q75`), y = b_sigma,  label = paste(sprintf("%0.4f", round(`Q50`, digits = 4))," [", sprintf("%0.4f", round(`Q2.5`, digits = 4)), "," , sprintf("%0.4f", round(`Q97.5`, digits = 4)), "]", sep = "")))%>%
    + scale_x_continuous(limits=sd_scale, breaks = seq(-1, 1, by = 0.1), expand = c(0,0))%>%
    + scale_y_discrete(expand = c(0.01,0))%>%
...
gg1 <- get_coef_plot(factor_abserror_fit, "Abs Error",c(-0.1, 0.1), c(-.1, .1), ABSERROR_COLOR)
gg2 <- get_coef_plot(factor_reactime_fit, "Reaction Time", c(-0.1, 0.2), c(-0.1, 0.2), REACT_COLOR)

ggsave(file = "pic/fig_interaction_effects_mu.pdf", ggarrange(gg1[[1]], gg2[[1]], widths=c(1,1)), width = 10, height = 4.2, units = "in")
ggsave(file = "pic/fig_interaction_effects_sigma.pdf", ggarrange(gg1[[2]], gg2[[2]], widths=c(1,1)), width = 10, height = 4.2, units = "in")

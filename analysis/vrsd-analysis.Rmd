...
draw_post <- function( m, 
  ...
  post_samples <- trials %>%
    ...
    mutate(levels_names = factor(levels_names, levels = rev(target_order)), levels_values = factor(levels_values), sample_value = exp(sample_value))
...
```{r RQ4, fig.height=12, fig.width=12, warning=FALSE, message=FALSE}
 ...
 draw_all <- function( m, 
  ...
  g <- post_samples %>%
      ...
      geom_richtext(data = b_CIs, mapping = aes(x = tail(breaks_x, n = 1) * 2.2, 
                                                y = Condition, 
                                                label = CIs), 
                 label.color = NA,
                 color = TEXT_COLOR,
      ...
      scale_x_continuous(breaks = breaks_x, expand = expansion(mult = c(0.01, 1.2))) +
      coord_cartesian(xlim = c(breaks_x[1], tail(breaks_x, n = 1))) +
      ggtitle(title) + xlab("") + ylab("") 
  g
  return(g)
...
ggsave(g_combined_rq4, filename = "pic/RQ4-all.pdf", width = 14, height = 11, units = "in")
...
```{r RQ4-tiles, fig.height=12, fig.width=12, warning=FALSE, message=FALSE}
 ...
 draw_tiles <- function( m, 
  ...
  post_samples <- trials %>%
    ...
    ungroup()%>%
    select(Device, Motion, Projection, Shading, Dimensionality, DataModel, sample_value) %>%
    mutate(  Device = recode(Device,  `0.5` = "2_HMD", `-0.5` = "1_Desktop"),
             Motion = recode(Motion,  `0.5` = "2_Motion", `-0.5` = "1_No Motion"),
             Projection = recode(Projection, `-0.5` = "1_Orthographic", `0.5` = "2_Perspective"),
             Shading = recode(Shading, `-0.5` = "1_Flat Shading", `0` = "2_Simple", `0.5` = "3_Ambient Occlusion"),
             DataModel = recode(DataModel, `0.5` = "2_CIFAR", `-0.5` =  "1_bAbI"),
             Dimensionality = recode(Dimensionality, `-0.5` = "2D t-SNE",  `0.5` = "3D t-SNE"), 
             sample_value = exp(sample_value),
  ...
  g <- b_CIs %>%
      ...
      geom_circle(aes(color = lower95, x0 = 47, y0 = scale_factor * index, r = adjust_in * sqrt(lower95)), fill = NA, size = 0.1) +
...
ggsave(g_combined_rq4_tiles, filename = "pic/RQ4_tiles.pdf", width = 10.5, height = 19.5, units = "in")

...
meta_path <- dir("participants_data/meta", full.names = T) #where you have your files
metas <- do.call(rbind,lapply(meta_path,read.csv.row_null))

trials_path <- dir("participants_data/judgments", full.names = T) #where you have your files
trials <- do.call(rbind,lapply(trials_path,read.csv.row_null))
trials <- trials[, !(names(trials) %in% c("X")), drop = T] #useless columns

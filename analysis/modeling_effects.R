...
get_coef_plot <- function(factor_fit, ytitle, mu_scale, sd_scale, fcolor){
  ...
  g_var_mu <- g %>%
    ...
    + geom_text(data = coefs_ind, size = 2, aes(x = max(`Q97.5`) * 1.5, y = b_ind,  label = paste(sprintf("%0.4f", round(`Q50`, digits = 4))," [", sprintf("%0.4f", round(`Q2.5`, digits = 4)), "," , sprintf("%0.4f", round(`Q97.5`, digits = 4)), "]", sep = "")))%>%
    + scale_x_continuous(limits=mu_scale, breaks = seq(-0.5, 0.5, by = 0.1), expand = c(0.01,0.01))%>%
...
gg2 <- get_coef_plot(factor_reactime_fit, "Reaction Time", c(-0.01, 0.41), c(-0.01, 0.41), REACT_COLOR)

ggsave(file = "pic/fig_effects_mu.pdf", ggarrange(gg1[[1]], gg2[[1]], widths=c(1,1)), width = 7, height = 2.25, units = "in")
ggsave(file = "pic/fig_effects_sigma.pdf", ggarrange(gg1[[2]], gg2[[2]], widths=c(1,1)), width = 7, height = 2.5, units = "in")
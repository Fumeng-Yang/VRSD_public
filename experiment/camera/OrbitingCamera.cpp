...
OrbitingCamera::OrbitingCamera() {
   m_aspectRatio = S.camera_aspectRatio;
   m_angleX      = m_angleY = 0;
   m_zoomZ       = -5;
}
...
void OrbitingCamera::updateProjectionMatrix() {
   ...
   float farPlane = S.camera_far;//std::max(settings.cameraFar, settings.cameraNear + 100.f * FLT_EPSILON);
   float h        = farPlane * glm::tan((S.camera_fov) / 180.0f * 3.14);
   float w        = m_aspectRatio * h;

   m_scaleMatrix      = glm::scale(glm::vec3(1.f / w, 1.f / h, 1.f / farPlane));
   m_projectionMatrix = glm::perspective(glm::radians(S.camera_fov), m_aspectRatio, S.camera_near, S.camera_far);
